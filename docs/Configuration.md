...
1. Generate the secret in gh action, like we do for the postgresql password. Add a new key value pair to `parameters.secrets.value` in [deploy.Bicep.ps1](../.azure/deployBicep.ps1), for example `myShinyNewPassword`:
    ```yml
    ...
