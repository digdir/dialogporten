...
internal sealed class DialogContentValueDtoValidator : AbstractValidator<DialogContentValueDto>, IIgnoreOnAssemblyScan
{
    public DialogContentValueDtoValidator(DialogContentType dialogContentType)
    {
        RuleFor(x => x.MediaType)
            .NotEmpty()
            .Must(value => value is not null && dialogContentType.AllowedMediaTypes.Contains(value))
            .WithMessage($"{{PropertyName}} '{{PropertyValue}}' is not allowed for content type {dialogContentType.Name}. " +
                         $"Allowed media types are {string.Join(", ", dialogContentType.AllowedMediaTypes.Select(x => $"'{x}'"))}");
        ...
        RuleFor(x => x.Value)
            .NotEmpty()
            .SetValidator(_ => new LocalizationDtosValidator(dialogContentType.MaxLength));
    }
}
