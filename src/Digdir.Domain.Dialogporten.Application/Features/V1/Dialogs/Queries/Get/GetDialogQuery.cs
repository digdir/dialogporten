...
internal sealed class GetDialogQueryHandler : IRequestHandler<GetDialogQuery, OneOf<GetDialogDto, EntityNotFound>>
{
    ...
    public async Task<OneOf<GetDialogDto, EntityNotFound>> Handle(GetDialogQuery request, CancellationToken cancellationToken)
    {
        ...
        var dialog = await _db.Dialogs
            .Include(x => x.Body.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.Title.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.SenderName.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.SearchTitle.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.Elements.OrderBy(x => x.CreatedAt))
                .ThenInclude(x => x.DisplayName.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.Elements.OrderBy(x => x.CreatedAt))
                .ThenInclude(x => x.Urls.OrderBy(x => x.CreatedAt))
            .Include(x => x.GuiActions.OrderBy(x => x.CreatedAt))
                .ThenInclude(x => x.Title.Localizations.OrderBy(x => x.CreatedAt))
            .Include(x => x.ApiActions.OrderBy(x => x.CreatedAt))
                .ThenInclude(x => x.Endpoints.OrderBy(x => x.CreatedAt))
            .AsNoTracking()
